...
public class PluginImplTest {
  ...
  public void beforeTest() throws IOException{
    ...
    String buildString = """
        plugins {
          id 'me.karjan.enumvisitorgenerator'
        }
        """;
  ...
  public void testGenerateVisitors_compileJavaDependency() throws IOException {
    tempProjectDir.newFolder("src", "main", "visgen");
  ...
  public void testGenerateVisitors_ignoreWrongFileExtention() throws IOException {
    File tempSourceDir = tempProjectDir.newFolder("src", "main", "visgen", "pkgZ");
    ...
    File expectedGenFile = new File(tempProjectDir.getRoot() + "/build/generated-src/visgen/pkgZ", "Other.txt");
  ...
  public void testGenerateVisitors_withEmptyEnumName() throws IOException {
    ...
    Assert.assertTrue("Build result output does not contain expected string: " + expectedOutput,
  ...
  public void testGenerateVisitors_withInvalidLocation() throws IOException {
    File tempSourceDir = tempProjectDir.newFolder("src", "main", "visgen");
  ...
  public void testGenerateVisitors_withValidDefinition() throws IOException {
    File tempSourceDir = tempProjectDir.newFolder("src", "main", "visgen", "pkgA", "pkg1");
    ...
    File expectedGenFile = new File(tempProjectDir.getRoot() + "/build/generated-src/visgen/pkgA/pkg1", testEnumName + JAVA_FILE_EXT);
    verifyJavaFile(expectedGenFile);
  }

  private static void writeVisitorDefinition(File directory, String baseFileName) throws IOException {
    String visitorDefinition = """
        ValidMember
        """;
  ...
  private static void writeVisitorDefinition(File directory, String baseFileName, String visitorDefinition) throws IOException {
  ...
  private static void verifyJavaFile(File javaFile) throws IOException {
    Assert.assertTrue("The file must exist -- " + javaFile.toString(),
        javaFile.exists());

    String fileName = javaFile.getName();

    Assert.assertTrue("The file name must end with '" + JAVA_FILE_EXT + "'",
        fileName.endsWith(JAVA_FILE_EXT));
    ...
    Assert.assertTrue("The file contents must contain '" + expectedPackage + "'",
        fileContents.contains(expectedPackage));

    String expectedEnum = "enum " + enumName;
    Assert.assertTrue("The file contents must contain '" + expectedEnum + "'",
        fileContents.contains(expectedEnum));

    String expectedInterface = "public interface " + enumName + "Visitor";
    Assert.assertTrue("The file contents must contain '" + expectedInterface + "'",
